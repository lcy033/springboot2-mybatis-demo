        ...
        for (int i = 0; i < 11; i++) {
            for (int j = 0; j < 11; j++) {
                if (chessArr1[i][j] != 0) {
                    ...
                    //代表的是第几个棋子。  spareseArr[count]中存放的一维数组中存放的内容就是这个棋子的信息。
                    count++;
                    //第一列，存放（非零数据）棋子在普通数组（棋盘）中的位置（行数）
                    spareseArr[count][0] = i;
                    //第二列，存放（非零数据）棋子在普通数组（棋盘）中的位置（列数）
                    spareseArr[count][1] = j;
                    //第三列，存放（非零数据）数据。（是黑棋蓝棋）
